...
def train(
    ...
    classifier_weights = ResNet50_Weights.DEFAULT
    classifier = resnet50(weights=classifier_weights)
    classifier.to("cuda")
    classifier.eval()

    classifier_preprocess = classifier_weights.transforms()

    erase_concept_idx = classifier_weights.meta["categories"].index(erase_concept)
    ...
    if len(erase_from) != len(erase_concept):
        if len(erase_from) == 1:
            ...
            raise Exception(
                "Erase from concepts length need to match erase concepts length"
            )
    ...
    with finetuner:
        ...
        for i in pbar:
            with torch.no_grad():
                ...
                positive_text_embeddings = diffuser.get_text_embeddings(
                    [erase_concept_sampled[0]], n_imgs=1
                )
                target_text_embeddings = diffuser.get_text_embeddings(
                    [erase_concept_sampled[1]], n_imgs=1
                )
                ...
                latents_steps_nsteps, _ = diffuser.diffusion(
                    latents,
                ...
                latents_steps_iteration, _ = diffuser.diffusion(
                    latents,
                    positive_text_embeddings,
                    start_iteration=0,
                    end_iteration=iteration,
                ...
                image = diffuser.to_image(
                    diffuser.decode(latents_steps_nsteps[0].to("cuda"))
                )

                classifier_score = classifier(
                    classifier_preprocess(image[0]).cuda().unsqueeze(0)
                )[:, erase_concept_idx].item()

            epsilon = diffuser.get_noise(batch_size=1, img_size=512)
            positive_latents = diffuser.predict_noise(
                iteration,
                latents_steps_iteration[0],
                positive_text_embeddings,
                guidance_scale=1,
            )

            loss = -criteria(epsilon, positive_latents) * classifier_score
            print(f"Loss: {loss.item()} Classifier Score: {classifier_score}")
    ...
    del (
        ...
        finetuner,
        positive_latents,
        latents_steps_nsteps,
        latents_steps_iteration,
        latents,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--erase_from",
        help="target concept to erase from",
        type=str,
        required=False,
        default=None,
    )
    ...
    parser.add_argument(
        "--iterations", help="Number of iterations", type=int, default=200
    )
    parser.add_argument("--lr", help="Learning rate", type=float, default=2e-5)
    parser.add_argument(
        "--negative_guidance",
        help="Negative guidance value",
        type=float,
        required=False,
        default=1,
    )
    parser.add_argument(
        "--save_path", help="Path to save model", type=str, default="models_new/"
    )
    parser.add_argument(
        "--device",
